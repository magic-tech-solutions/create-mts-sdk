...
const createProject = async (projectName) => {
  ...
  try {
    ...
    await installNodeModules(projectName, spinner, runtime);
    // isUpdated && (await updateNodeModules(projectName, spinner));
    // isDeduped && (await dedupeNodeModules(projectName, spinner));
    await postInstallScripts(projectName, template, spinner, runtime);
...
const chooseRuntime = async () => {
    const { chooseRuntime } = await inquirer.prompt([
      {
        type: "list",
        name: "chooseRuntime",
 ...
 */
const installNodeModules = async (destination, spinner, runtime) => {
  spinner.text = `Install node_modules using ${runtime}...\n`;
  let installCommand = "";
 ...
 */
const postInstallScripts = async (destination, template, spinner, runtime) => {
    let prismaGenerateCommand = "";

    switch (runtime) {
      case "npm":
        prismaGenerateCommand = "npm run prisma:generate";
        break;
      case "yarn":
        prismaGenerateCommand = "yarn prisma:generate";
        break;
      case "pnpm":
        prismaGenerateCommand = "pnpm run prisma:generate";
        break;
      case "bun":
        prismaGenerateCommand = "bun run prisma:generate";
        break;
      default:
        prismaGenerateCommand = "npm run prisma:generate"; // Fallback to npm
        break;
    }
